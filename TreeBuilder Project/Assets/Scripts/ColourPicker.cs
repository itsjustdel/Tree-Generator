...
public class ColourPicker : MonoBehaviour {
    ...
    public bool applyToCubes = false;
    ...
    public void ChooseRandom()
    {
        ...
        if (!useGlobalSaturation)
        {
            ...
            float satChooser = Random.Range(0.5f, .75f);//still playin with these
        ...
        float valChooser = Random.Range(0.33f, .66f);//overide atm
         value = valChooser;
        value = 1f;//this really just controls how much light is looking like it is hitting it, perhaps should be a constant


        harmonyStep = 30 * Random.Range(1, 5);

        //add to publicly accessed class/list
        matsAndShades = Palette(hue, saturation, value);
    }
    ...
    MaterialAndShades ColourAndShades(float hueP,float saturationP,float valueP)
    {
        ...
        standardMaterial.SetFloat("_Metallic", 0.15f);
        standardMaterial.SetFloat("_Glossiness", 0.15f);
